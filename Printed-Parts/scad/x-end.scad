...
raio_interno_porca = 5.4;
raio_externo_porca = 11.1;
raio_furos_porca = 8.3;
raio_furos_parafusos = 1.7;
...
//Nut trap
 // Cylinder
   //translate(v=[0,-17,0]) poly_cylinder(h = 8, r=12.5, $fn=25);
    //translate(v=[0,-17,0]) poly_cylinder(h = 8, r=12.5-offset_furo_porca, $fn=25);
    translate(v=[0,-17,0]) poly_cylinder(h = 8, r=raio_externo_porca, $fn=25);
 // Hexagon
...
// TR Nut trap
   // Hole for the nut
    //translate(v=[0,-17, -1]) poly_cylinder(h = 9.01, r = 6.6, $fn = 25); //  Bruno
    translate(v=[0,-17, -1]) poly_cylinder(h = 9.01, r = raio_interno_porca, $fn = 25); //  Bruno

// Screw holes for TR nut
    //translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 9.5, -1]) cylinder(h = 10, r = 1.55, $fn=25);
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, raio_furos_porca, -1]) cylinder(h = 10, r = raio_furos_parafusos, $fn=25); // Bruno
...
// Nut traps for TR nut screws
    ...
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, raio_furos_porca, 6]) rotate([0, 0, 0])cylinder(h = 3, r = 3.3, $fn=6); // Bruno
    
    //translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -9.5, 6]) rotate([0, 0, 30])cylinder(h = 3, r = 3.2, $fn=6);
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, -raio_furos_porca-0.2, 6]) rotate([0, 0, 30])cylinder(h = 3, r = 3.2, $fn=6); // Bruno
    
    //translate([-5.5,-17.2,6]) rotate([0,0,30]) cube([5,5,3]);
    translate([-3.70,-17.2,6]) rotate([0,0,45]) cube([5,5,3]); // Bruno
    
    //translate([-0,-17.2,6]) rotate([0,0,60]) cube([5,10,3]);
    translate([-0,-20.18,6]) rotate([0,0,45]) cube([5,10,3]); // Bruno
}
